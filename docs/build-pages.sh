...
# Build CONFIG-REFERENCE page too
if [ -f "CONFIG-REFERENCE.md" ]; then
  echo "Converting CONFIG-REFERENCE.md to HTML..."
  CONFIG_CONTENT=$(cd /tmp && /home/ihoner/.nvm/versions/node/v22.21.1/bin/npx --yes marked --gfm < "$OLDPWD/CONFIG-REFERENCE.md")

  # Reuse same template
  cat > "$OUTDIR/config-reference.html" << 'CFGHEADER'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<div class="layout">
  <nav class="sidebar">
    <div class="sidebar-hdr"><span class="logo">🦞 Cortex</span><span class="badge">CONFIG</span></div>
...
CFGHEADER
  ...
  echo "$CONFIG_CONTENT" >> "$OUTDIR/config-reference.html"

  cat >> "$OUTDIR/config-reference.html" << 'CFGFOOTER'
  </main>
</div>
<button class="top-btn" id="top" onclick="scrollTo({top:0,behavior:'smooth'})">↑</button>
<script>
...
CFGFOOTER
  ...
  echo "   Config ref: $(du -sh "$OUTDIR/config-reference.html" | cut -f1)"
fi

# Build PLAN page too
if [ -f "PLAN.md" ]; then
  echo "Converting PLAN.md to HTML..."
  PLAN_CONTENT=$(cd /tmp && /home/ihoner/.nvm/versions/node/v22.21.1/bin/npx --yes marked --gfm < "$OLDPWD/PLAN.md")

  # Reuse same template
  cat > "$OUTDIR/plan.html" << 'PLANHEADER'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Cortex Project Plan</title>
  <style>
    :root {
      --bg: #0a0e1a; --bg2: #111827; --card: #1a1f2e; --code-bg: #151b2d;
      --text: #e2e8f0; --text2: #94a3b8; --muted: #64748b;
...
<div class="layout">
  <nav class="sidebar">
    <div class="sidebar-hdr"><span class="logo">🦞 Cortex</span><span class="badge">PLAN</span></div>
    <a class="back-link" href="index.html">← Back to Main Guide</a>
    <ul class="toc" id="toc"></ul>
  </nav>
  <main class="main" id="content">
PLANHEADER
  ...
  echo "$PLAN_CONTENT" >> "$OUTDIR/plan.html"

  cat >> "$OUTDIR/plan.html" << 'PLANFOOTER'
  </main>
</div>
<button class="top-btn" id="top" onclick="scrollTo({top:0,behavior:'smooth'})">↑</button>
<script>
...
PLANFOOTER
  ...
  echo "   Project plan: $(du -sh "$OUTDIR/plan.html" | cut -f1)"
fi
